...
EXT_CODE_LIST = [
    ...
    'SS',
    'SCM']
EXT_EXECUTABLE_LIST = ['LNK', 'DEB', 'EXE', 'SH', 'BUNDLE', 'MSI'	]
EXT_COMPRESSED_LIST = [
    'RAR',
    'JAR',
    'ZIP',
...
def Organize(dirs, name):
    ...
    dest = r'{}\{}'.format(destLocation, name)
...
for dirs in os.listdir(os.getcwd()):
    if 1:
        for name, extensions_list in zip(
            TYPES_LIST, [
                EXT_VIDEO_LIST, EXT_IMAGE_LIST, EXT_DOCUMENT_LIST, EXT_MUSIC_LIST, EXT_CODE_LIST, EXT_EXECUTABLE_LIST, EXT_COMPRESSED_LIST]):
