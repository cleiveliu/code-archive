# source:
# https://github.com/geekcomputers/Python/blob/master/wiki_random.py
...
page_count = 10
url = 'https://en.wikipedia.org/w/api.php?action=query&list=random&rnnamespace=0&rnlimit=' + \
...
def load():
    ...
    if response.ok:
        ...
        i = input(
            "Which page you want to see, enter index..[r: for retry,n: exit]?").lower()
        if i == 'r':
            ...
            webbrowser.get().open('https://en.wikipedia.org/wiki?curid=' +
                                  str(jsonData[int(i)]['id']))
        load()
    else:
